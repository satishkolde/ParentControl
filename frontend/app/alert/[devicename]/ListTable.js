...
export default function ListTable(props) {
  ...
  return (
    <>
      <div className="relative flex flex-col min-w-0 break-words bg-white mb-2 shadow-lg rounded">
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1">
              <h3 className="font-semibold text-base text-blueGray-700">
                {props.tableTitle} List
              </h3>
            </div>
          </div>
        </div>
                    ...
                    <tr key={index}>
                        ...
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                            <div className="flex items-center">
                                <span className="mr-2">{((row.count * 100) / props.totalCount).toFixed(1)}%</span>
                                <div className="relative w-full">
                                    <div className="overflow-hidden h-2 text-xs flex rounded bg-red-200">
                                        <div
                                            style={{ width: `${(row.count * 100)/props.totalCount}%` }}
